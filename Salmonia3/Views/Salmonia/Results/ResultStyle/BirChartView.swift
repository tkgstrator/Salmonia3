...
struct BarChartView: View {
    ...
    func barResult(salmonId: SalmonidType, index: Int) -> some View {
        HStack(alignment: .center, spacing: nil, content: {
            ...
            VStack(alignment: .leading, spacing: 0, content: {
                Rectangle()
                    ...
                    .overlay(Text("\(bossCounts[index])").splatfont2(.white, size: 16).shadow(color: .black, radius: 0, x: 2, y: 2))
    ...
    var barChart: some View {
        ForEach(Array(zip(SalmonidType.allCases.indices, SalmonidType.allCases)), id:\.0) { index, salmonId in
            if bossCounts[index] > 0 {
                barResult(salmonId: salmonId, index: index)
            }
        }
    }
